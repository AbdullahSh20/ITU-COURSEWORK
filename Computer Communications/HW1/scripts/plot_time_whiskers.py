...
time_data_list = [Morning_data, Afternoon_data, Night_data]

for time_data in time_data_list:
    average_delay = time_data['Delay (ms)'].mean()
    std_deviation = time_data['Delay (ms)'].std()
    average_delays.append(average_delay)
    std_deviations.append(std_deviation)

plt.bar(time_labels, average_delays, color=bar_colors, yerr=std_deviations, capsize=5)
